...
def get_filtered_eeg(
    eeg, lowcut, highcut, order, sample_rate, onset=38, delay=0.135, signal_end=4
):
    """
    ...
    trial_len = int(onset + delay * sample_rate + signal_end * sample_rate - 1) - int(
        onset + delay * sample_rate
    )
    ...
    for target in range(0, num_classes):
        for channel in range(0, num_chan):
            for trial in range(0, num_trials):
                signal_to_filter = np.squeeze(
                    eeg[
                        target,
                        channel,
                        int(onset + delay * sample_rate) : int(
                            onset + delay * sample_rate + signal_end * sample_rate - 1
                        ),
                        trial,
                    ]
                )
                filtered_data[target, channel, :, trial] = butter_bandpass_filter(
                    signal_to_filter, lowcut, highcut, sample_rate, order
                )
...
def buffer(data, duration, data_overlap):
    ...
    number_segments = int(
        math.ceil((len(data) - data_overlap) / (duration - data_overlap))
    )
    temp_buf = [
        data[i : i + duration]
        for i in range(0, len(data), (duration - int(data_overlap)))
    ]
...
def get_segmented_epochs(data, window_len, shift_len, sample_rate):
    ...
    number_of_segments = int(
        math.ceil((data.shape[2] - data_overlap) / (duration - data_overlap))
    )

    segmented_data = np.zeros(
        (data.shape[0], data.shape[1], data.shape[3], number_of_segments, duration)
    )

    for target in range(0, num_classes):
        for channel in range(0, num_chan):
            for trial in range(0, num_trials):
                segmented_data[target, channel, trial, :, :] = buffer(
                    data[target, channel, :, trial], duration, data_overlap
                )
...
def magnitude_spectrum_features(segmented_data, FFT_PARAMS):
    ...
    fft_index_start = int(
        round(FFT_PARAMS["start_frequency"] / FFT_PARAMS["resolution"])
    )
    fft_index_end = (
        int(round(FFT_PARAMS["end_frequency"] / FFT_PARAMS["resolution"])) + 1
    )

    features_data = np.zeros(
        (
            (fft_index_end - fft_index_start),
        ...
        for channel in range(0, num_chan):
            for trial in range(0, num_trials):
                for segment in range(0, number_of_segments):
                    temp_FFT = (
                        np.fft.fft(
                            segmented_data[target, channel, trial, segment, :], NFFT
                        )
                        / fft_len
                    )
                    magnitude_spectrum = 2 * np.abs(temp_FFT)
                    features_data[:, channel, target, trial, segment] = (
                        magnitude_spectrum[fft_index_start:fft_index_end,]
                    )
...
def complex_spectrum_features(segmented_data, FFT_PARAMS):
    ...
    features_data = np.zeros(
        (
            2 * (fft_index_end - fft_index_start),
            segmented_data.shape[1],
            segmented_data.shape[0],
            segmented_data.shape[2],
            segmented_data.shape[3],
                ...
                for segment in range(0, number_of_segments):
                    ...
                    features_data[:, channel, target, trial, segment] = np.concatenate(
                        (
                            ...
                            imag_part[fft_index_start:fft_index_end,],
                        ),
                        axis=0,
                    )
