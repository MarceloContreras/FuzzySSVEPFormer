        ...
        for seed_num, ckpt in enumerate(checkpoints):
            ...
            for noise in SIGMAS:
                ...
                if args.save_latent:
                    ...
                    np.save(
                        f"latent_space/X_{args.model}_{args.dataset}_S{sub+1}_n{noise}_seed{seed_num}",
                        X_latent,
                    )
                    np.save(
                        f"latent_space/Y_{args.model}_{args.dataset}_S{sub+1}_n{noise}_seed{seed_num}",
                        Y_latent_space,
                    )
