...
def get_args_parser():
    ...
    parser.add_argument("--test_noise", action="store_true", default=False, help="")
    parser.add_argument("--test_time", action="store_true", default=False, help="")
    parser.add_argument(
...
def main(args):
    ...
    model_name = (
        f"{args.model}_{args.signal_size:.1f}s_{args.dataset}_S{args.sub+1}.pth"
    )
            ...
            match args.dataset:
                case "NAKANISHI":
                    datahandler = NakanishiHandler(
                        params, args.signal_size, args.data_path
                    )
                case "BENCHMARK":
                ...
                case "WEARABLE_WET":
                    datahandler = WearableHandlerWet(
                        params, args.signal_size, args.data_path
                    )
                case "WEARABLE_DRY":
                    datahandler = WearableHandlerDry(
                        params, args.signal_size, args.data_path
                    )
